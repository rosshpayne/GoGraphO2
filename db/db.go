...
func SaveChildUIDtoOvflBlock(cUID, tUID util.UID, sortk string, id int) error { //
	...
	// add child-uid to overflow block. Consider using SIZE on attribute ND to limit number of RCUs that need to be consumed.
	// if size was restricted to 100K or 25 RCS that woould be quicker and less costly than 400K item size.
	// Consume more overflow blocks to reduce IO on data propagation.
	//
	v := make([][]byte, 1, 1)
	v[0] = []byte(cUID)
	upd = expression.Set(expression.Name("Nd"), expression.ListAppend(expression.Name("Nd"), expression.Value(v)))
	cond := expression.Name("XF").Size().LessThanEqual(expression.Value(param.OvfwNdLimit))
	//
...
func newUIDTarget(tUID util.UID, sortk string, id int) (map[string]*dynamodb.AttributeValue, error) { // create dummy item with flag value of DELETED. Why? To establish Nd & XF attributes as Lists rather than Sets..
	...
	//
	xf := make([]int, 1, 1)
	xf[0] = blk.UIDdetached // this is a nil entry so mark it deleted. Used to append other cUIDs too.
	//
	// create sortk value
	//
	var s strings.Builder
...
func EdgeExists(cuid, puid util.UID, sortk string, action byte) (bool, error) {
	...
	//
	//
	//TODO: create a NodeExists() to check if UID exists in cache/db. Currently it presumes it exists and will continue to AttachNode...
	//
	...
	case DELETE:
		...
		upd = expression.Delete(expression.Name("PBS"), expression.Value(pbs)) // X is just a dummy to be replaced later.
	...
	case ADD:
		...
		pbsC = append(puid, pred(sortk)...)
		//
		upd = expression.Add(expression.Name("PBS"), expression.Value(pbs)) // X is just a dummy to be replaced later.
		cond = expression.Contains(expression.Name("PBS"), "X").Not()
		// workaround: as expression will want Compare(predicate,<string>), but we are comparing Binary will change ExpressionAttributeValue to use binary attribute value.
